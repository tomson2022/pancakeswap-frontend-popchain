...
const fetchTopPools = async (chainName: MultiChainName, timestamp24hAgo: number): Promise<string[]> => {
  ...
  if (!isStableSwap) {
    switch (chainName) {
      case 'BSC':
        whereCondition = `where: { dailyTxns_gt: 300, token0_not_in: $blacklist, token1_not_in: $blacklist, date_gt: ${timestamp24hAgo} }`
        break
      case 'POPCHAIN':
        ...
        break
      default:
        whereCondition = `where: { date_gt: ${timestamp24hAgo}, token0_not_in: $blacklist, token1_not_in: $blacklist, dailyVolumeUSD_gt: 2000 }`
    }
  }
  try {
    const query = gql`
    ...
    const poolAddresses = data.pairDayDatas.map((p) => p.id.split('-')[0])
