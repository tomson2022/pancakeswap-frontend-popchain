...
interface OverviewResponse {
  ...
  uniswapFactories?: PancakeFactory[]
}
/**
...
const getOverviewData = async (
  ...
  let factoryString = checkIsStableSwap() ? `factories` : `pancakeFactories`
  let queryFields = `
    totalTransactions
    totalVolumeUSD
    totalLiquidityUSD
...
const formatPancakeFactoryResponse = (rawPancakeFactory?: PancakeFactory) => {
  if (rawPancakeFactory) {
    return {
      totalTransactions: rawPancakeFactory.totalTransactions ? parseFloat(rawPancakeFactory.totalTransactions) : 0,
...
const useFetchProtocolData = (): ProtocolFetchState => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const overviewData = formatPancakeFactoryResponse(getFactoryData(data))
      const overviewData24 = formatPancakeFactoryResponse(getFactoryData(data24))
      const overviewData48 = formatPancakeFactoryResponse(getFactoryData(data48))
