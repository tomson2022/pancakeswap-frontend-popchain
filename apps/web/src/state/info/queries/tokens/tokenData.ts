...
const parseTokenData = (tokens?: TokenFields[], chainName?: MultiChainName) => {
  if (!tokens) {
    return {}
  }
  return tokens.reduce((accum: { [address: string]: FormattedTokenFields }, tokenData) => {
    ...
    // PopChain 只有 derivedETH，没有 derivedUSD 和 totalTransactions
    if (chainName === 'POPCHAIN') {
      ...
      accum[tokenData.id.toLowerCase()] = {
        ...
        derivedETH: derivedETH ? parseFloat(derivedETH) : 0,
        derivedUSD: parseFloat(derivedUSD || '0'),
        tradeVolumeUSD: parseFloat(tradeVolumeUSD || '0'),
        totalTransactions: parseFloat(totalTransactions || '0'),
        totalLiquidity: parseFloat(totalLiquidity || '0'),
      }
    }
    return accum
  ...
  useEffect(() => {
    const fetch = async () => {
      ...
      if (error) {
        ...
        const parsed = parseTokenData(data?.now, chainName)
        const parsed24 = parseTokenData(data?.oneDayAgo, chainName)
        const parsed48 = parseTokenData(data?.twoDaysAgo, chainName)
        const parsed7d = parseTokenData(data?.oneWeekAgo, chainName)
        const parsed14d = parseTokenData(data?.twoWeeksAgo, chainName)
