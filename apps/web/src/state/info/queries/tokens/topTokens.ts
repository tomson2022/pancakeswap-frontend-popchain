...
const fetchTopTokens = async (chainName: MultiChainName, timestamp24hAgo: number): Promise<string[]> => {
  ...
  if (chainName === 'POPCHAIN') {
    try {
      ...
      `
      const data = await getMultiChainQueryEndPointWithStableSwap(chainName).request<{ tokens: { id: string }[] }>(query)
      return data.tokens.map((t) => t.id)
    } catch (error) {
      console.warn('fetchTopTokens for PopChain', { chainName, timestamp24hAgo })
      console.error('Failed to fetch top tokens for PopChain', error)
      return []
    }
  }

  // 其他链使用原来的 tokenDayDatas 逻辑
  let whereCondition = ''
  ...
  if (checkIsStableSwap()) {
    ...
    switch (chainName) {
      case 'ETH':
        whereCondition = `where: { date_gt: ${timestamp24hAgo}, token_not_in: $blacklist, dailyVolumeUSD_gt:2000 }`
        break
      default:
        whereCondition = `where: { dailyTxns_gt: 300, id_not_in: $blacklist, date_gt: ${timestamp24hAgo}}`
    }
  }
