...
export const useLPApr = (pair?: Pair) => {
  const { data: poolData } = useSWRImmutable(
    pair && pair.chainId === ChainId.POPCHAIN ? ['LP7dApr', pair.liquidityToken.address] : null,
    async () => {
      const timestampsArray = getDeltaTimestamps()
      const blocks = await getBlocksFromTimestamps(timestampsArray, 'desc', 1000, 'POPCHAIN')
...
const POOL_AT_BLOCK = (chainName: MultiChainName, block: number | null, pool: string) => {
  ...
  // PopChain 使用 reserveUSD 字段，其他链使用 trackedReserve 字段
